...
for (let row = 0; row < level1.length; row++) {
  for (let col = 0; col < level1[row].length; col++) {
    ...
    createBubble(startX + col * (grid + bubbleGap + 2), row * (grid + bubbleGap), color);
  }
}

const randomColor = Object.keys(colorMap)[Math.floor(Math.random() * Object.keys(colorMap).length)];
const playerBubble = {
  x: realCanvasWidth / 2 + margin,
...
function updatePlayerPosition(event) {
  ...
  mouseX = Math.max(margin, Math.min(event.clientX - rect.left - margin, realCanvasWidth));
...
function draw() {
  ...
  context.drawImage(customPointer, mouseX - 5, mouseY - 5, 55, 55);
}
